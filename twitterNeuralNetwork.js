...
function printNetworkCreateResultsHashmap(){
  ...
  async.each(Object.keys(networkCreateResultsHashmap), function(nnId, cb){
    ...
    const elapsed = (networkObj.evolve.elapsed !== undefined) ? networkObj.evolve.elapsed : (moment().valueOf() - networkObj.evolve.startTime);
...
function loadInputsDropboxFolder(folder, callback){
  ...
  loadDropboxFolder(options, function(err, results){
    ...
    console.log(chalkLog("NNT | DROPBOX LIST INPUTS FOLDER"
...
function generateRandomEvolveConfig (cnf, callback){
  ...
  if (!cnf.createTrainingSet && !cnf.createTrainingSetOnly && cnf.loadTrainingSetFromFile && trainingSetReady) {
    ...
    // console.log(chalkLog("NNT | TEST SET META\n" + jsonPrint(tObj.trainingSetObj.testSet.meta)));
    ...
    switch (cnf.networkCreateMode) {
      ...
      case "evolve":
        ...
        console.log(chalkBlue("NNT | TEST RUN ID: " + statsObj.tests[testObj.testRunId][nnId].testRunId
          ...
          + "\nNNT | SEED:                " + messageObj.seedNetworkId + " | " + messageObj.seedNetworkRes + "%"
...
function initNeuralNetworkChild(nnChildIndex, cnf, callback){
  ...
  neuralNetworkChildHashMap[nnChildId].child.on("error", function(err){
    ...
    console.log(chalkError("*** neuralNetworkChildHashMap " + nnChildId + " ERROR *** : " + err));
  });
  ...
  neuralNetworkChildHashMap[nnChildId].child.on("exit", function(err){
    ...
    console.log(chalkError("*** neuralNetworkChildHashMap " + nnChildId + " EXIT *** : " + err));
  });
...
initNetworkCreateInterval = function(){
  ...
  networkCreateInterval = setInterval(function(){
    ...
    if (initMainReady) {
      ...
      loadConfigFile(dropboxConfigHostFolder, dropboxConfigFile, function(err, configLoadedFlag){
        ...
        if (enableCreateChildren && (Object.keys(neuralNetworkChildHashMap).length < configuration.maxNeuralNetworkChildern)) {
