...
<p>The Gated Recurrent Unit network is very similar to the LSTM network. GRU networks have óne gate less and no selfconnections. Similarly to LSTM's, GRU's are well-suited to classify, process and predict time series when there are very long time lags of unknown size between important events.</p>
...
<p>The above network has 3 hidden layers, with 4 GRU assemblies each. It has two inputs and óne output.</p>
