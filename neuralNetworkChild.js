...
const DEFAULT_USER_PROFILE_CHAR_CODES_ONLY_INPUTS_ID =
  ...
  setInterval(async function () {
    if (readyToQuit()) {
      ...
      if (forceQuitFlag) {
        ...
        console.log(
          chalkBlueBold(
            MODULE_ID_PREFIX + " | ALL PROCESSES COMPLETE ... QUITTING"
          )
        );
      }
...
function updateTrainingSet(p) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      console.log(
        chalkError(MODULE_ID_PREFIX + " | *** updateTrainingSet ERROR:", err)
      );
      reject(err);
    }
  });
...
async function loadTrainingSet(p) {
  try {
    ...
    console.log(
      chalk.black.bold(
        MODULE_ID_PREFIX + " | loadTrainingSet | UPDATE TRAINING SET"
      )
    );
    ...
    console.log(
      chalkError(
        MODULE_ID_PREFIX + " | *** USERS TRAINING SET LOAD ERROR: " + err
      )
    );
    statsObj.loadUsersFolderBusy = false;
    statsObj.trainingSetReady = false;
...
function prepNetworkEvolve() {
    ...
    case "brain":
      ...
      options.schedule = function (schedParams) {
        ...
        const timeToComplete =
          iterationRate * (options.iterations - (schedParams.iterations + 1));
        ...
        console.log(
          chalkLog(
            MODULE_ID_PREFIX +
              ...
              sObj.error +
              " | R " +
              msToTime(sObj.evolveElapsed) +
              " | ETC " +
              msToTime(sObj.timeToComplete) +
              " " +
              moment().add(sObj.timeToComplete).format(compactDateTimeFormat) +
              " | " +
              (sObj.iterationRate / 1000.0).toFixed(1) +
              " spi" +
              " | I " +
              sObj.iteration +
              "/" +
              sObj.totalIterations
          )
        );
  ...
  if (
    ...
    console.log(
      chalkBlueBold(
        MODULE_ID_PREFIX + " | EVOLVE OPTIONS | SELECTION: " + options.selection
      )
    );
  ...
  if (options.cost !== undefined && typeof options.cost === "string") {
    console.log(
      chalkBlueBold(
        MODULE_ID_PREFIX + " | EVOLVE OPTIONS | COST: " + options.cost
      )
    );
  ...
  if (options.mutation !== undefined && typeof options.mutation === "string") {
    console.log(
      chalkBlueBold(
        MODULE_ID_PREFIX + " | EVOLVE OPTIONS | MUTATION: " + options.mutation
      )
    );
...
function dataSetPrep(params) {
  return new Promise(function (resolve, reject) {
    ...
    const numCharInputs =
      configuration.userCharCountScreenName +
  ...
  return new Promise(function (resolve, reject) {
    ...
    switch (childNetworkObj.architecture) {
      case "seed":
        ...
        if (
          !empty(childNetworkObj.networkRaw) &&
          childNetworkObj.networkRaw.evolve !== undefined
        ) {
          console.log(
            chalkLog(
              MODULE_ID_PREFIX +
          ...
          !empty(childNetworkObj.network) &&
          childNetworkObj.network.evolve !== undefined
        ) {
          console.log(
            chalkLog(
              MODULE_ID_PREFIX +
          ...
          if (childNetworkObj.networkTechnology === "carrot") {
            ...
            if (!empty(childNetworkObj.networkRaw)) {
              ...
              networkRaw = neataptic.Network.fromJSON(
            ...
            console.log(
              chalkError(
                MODULE_ID_PREFIX +
                  ...
                  " | *** CHILD NO RAW NETWORK: " +
                  childNetworkObj.seedNetworkId
              )
            );
            return reject(
              new Error("NO RAW NETWORK: " + childNetworkObj.networkId)
            );
          }
          // }
          // catch(err){
      ...
      case "perceptron":
        if (childNetworkObj.networkTechnology === "carrot") {
          if (
            ...
            childNetworkObj.hiddenLayerSize > 0
          ) {
            networkRaw = new carrot.architect.Perceptron(
              numInputs,
              childNetworkObj.hiddenLayerSize,
              3
            );
          ...
          if (childNetworkObj.hiddenLayerSize) {
            childNetworkObj.hiddenLayerSize = Math.min(
              configuration.brainHiddenLayerSize,
              childNetworkObj.hiddenLayerSize
            );
            childNetworkObj.hiddenLayerSize = Math.max(
              childNetworkObj.hiddenLayerSize,
              trainingSetObj.meta.numOutputs
            );
          } else {
          ...
          if (childNetworkObj.hiddenLayerSize) {
            childNetworkObj.hiddenLayerSize = Math.min(
              configuration.neatapticHiddenLayerSize,
              childNetworkObj.hiddenLayerSize
            );
            childNetworkObj.hiddenLayerSize = Math.max(
              childNetworkObj.hiddenLayerSize,
...
async function evolve(params) {
  ...
  try {
    ...
    if (
      childNetworkObj.networkTechnology !== "brain" &&
      evolveResults.iterations !== childNetworkObj.evolve.options.iterations
    ) {
      console.log(
        chalkError(
          MODULE_ID_PREFIX +
            " | *** EVOLVE ERROR: ITERATIONS" +
            " | EXPECTED: " +
            childNetworkObj.evolve.options.iterations +
            " | ACTUAL: " +
            evolveResults.iterations
        )
      );
    ...
    console.log(params);
    console.log(
      chalkError(MODULE_ID_PREFIX + " | *** EVOLVE ERROR: preppedOptions")
    );
...
function networkEvolve(p) {
  return new Promise(function (resolve, reject) {
    console.log(
      chalkBlueBold(
        MODULE_ID_PREFIX + " | NETWORK EVOLVE | " + configuration.childId
      )
    );
    ...
    if (params.seedNetworkId) {
      ...
      debug(
        chalkAlert(
          MODULE_ID_PREFIX + " | START NETWORK DEFINED: " + params.networkId
        )
      );
    }
      ...
      async function (err) {
        try {
          if (err) {
            console.log(
              chalkError(
                MODULE_ID_PREFIX + " | *** networkEvolve ERROR: " + err
              )
            );
            return reject(err);
          }
...
async function networkDefaults(nnObj) {
  try {
    ...
    console.log(
      chalkError(MODULE_ID_PREFIX + " | *** networkDefaults ERROR: " + err)
    );
    throw err;
  }
}
...
async function configNetworkEvolve(params) {
  try {
    ...
    console.log(
      chalkError(MODULE_ID_PREFIX + " | *** configNetworkEvolve ERROR: " + err)
    );
  }
}
  ...
  try {
    ...
    switch (m.op) {
      ...
      case "INIT":
        ...
        if (m.userProfileCharCodesOnlyFlag !== undefined) {
          configuration.userProfileCharCodesOnlyFlag =
            m.userProfileCharCodesOnlyFlag;
        }
      ...
      case "READY":
        console.log(
          chalkInfo(MODULE_ID_PREFIX + " | READY" + " | CHILD ID: " + m.childId)
        );
        fsm.fsm_ready();
        break;
    ...
    console.log(
      chalkError(MODULE_ID_PREFIX + " | *** PROCESS ON MESSAGE ERROR: " + err)
    );
  }
});
